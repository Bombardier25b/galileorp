...
function DrawTextOptions(carPos,currentName,currentDownpayment,currentCommission,swapCar,currentPrice,currentStock)
	if (PlayerData.job ~= nil and PlayerData.job.name == Config.CarDealerJobLabel) then
		-- info
		DrawText3Ds(carPos[1],carPos[2],carPos[3] + 0.6, "~c~"..currentName.."~s~ | ~o~$"..comma_value(math.floor(currentPrice + (currentPrice * (currentCommission / 100)))).."~s~ | ~p~"..currentCommission.."%~s~ Commission | ~b~"..currentStock.."~s~ PCS")
		-- buttons:
