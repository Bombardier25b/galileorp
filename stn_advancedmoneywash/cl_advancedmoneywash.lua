...
local elements = {
	...
	--{label = _U('slab_0'), value = 'slab_a'},
	--{label = _U('slab_1'), value = 'slab_b'},
	--{label = _U('slab_2'), value = 'slab_c'},
	--{label = _U('slab_3'), value = 'slab_d'}
	...
	exports.rprogress:Start("Dealing", 60000)
	ClearPedTasks(player)
    ClearPedSecondaryTask(player)
	FreezeEntityPosition(player,false)
	TriggerServerEvent('stn_advancedmoneywash:removeItem')
		...
		if data.current.value == 'wash_money' then
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'wash_money_amount_', {
				...
				if amount == nil then
					...
					menu.close()
					local startedjob = false
					TriggerServerEvent('stn_advancedmoneywash:washMoney', amount, zone)
...
RegisterNetEvent('stn_advancedmoneywash:checkpayandnotify')
AddEventHandler('stn_advancedmoneywash:checkpayandnotify', function(amount)
	TriggerServerEvent('stn_advancedmoneywash:sendToDiscord', amount)
end)

RegisterNetEvent("stn_advancedmoneywash:jobstart")
AddEventHandler("stn_advancedmoneywash:jobstart", function()
	...
	TriggerServerEvent('stn_advancedmoneywash:registerActivity', 1)

	TriggerServerEvent('stn_advancedmoneywash:giveItem')
    ...
    FreezeEntityPosition(player,true)
    TaskPlayAnim(player,anim_lib,anim_dict,1.0,0.5,-1,31,1.0,0,0)
	exports.rprogress:Start("Talking to the dealer", 10000)
...
Citizen.CreateThread(function()
	while true do
	  ...
	  if isTaken == 1 and IsPedInAnyVehicle(GetPlayerPed(-1)) then
		...
		TriggerServerEvent('stn_advancedmoneywash:alertcops', coords.x, coords.y, coords.z)
		  elseif isTaken == 1 and not IsPedInAnyVehicle(GetPlayerPed(-1)) then
			  TriggerServerEvent('stn_advancedmoneywash:stopalertcops')
	  end
	end
end)

  
AddEventHandler('stn_advancedmoneywash:hasEnteredMarker', function(zone)
...
AddEventHandler('stn_advancedmoneywash:hasExitedMarker', function(zone)
	CurrentAction = nil
	ESX.UI.Menu.CloseAll()	
...
Citizen.CreateThread(function()
	while true do
		...
		if isInMarker and not hasAlreadyEnteredMarker then
			...
			TriggerEvent('stn_advancedmoneywash:hasEnteredMarker', currentZone)
		end
		
		if not isInMarker and hasAlreadyEnteredMarker then
			hasAlreadyEnteredMarker = false
			TriggerEvent('stn_advancedmoneywash:hasExitedMarker', LastZone)
		end
		...
		if CurrentAction ~= nil then
			...
			if IsControlJustReleased(0, 38) then
				...
				if CurrentAction == 'jobmenu' then
					ESX.TriggerServerCallback('stn_advancedmoneywash:isActive', function(isActive, cooldown)
						
						if cooldown <= 0 then
						
							if isActive == 0 then
								ESX.TriggerServerCallback('stn_advancedmoneywash:anycops', function(anycops)
									if anycops >= Config.CopsRequired then
										TriggerEvent("stn_advancedmoneywash:jobstart")
									else
...
RegisterNetEvent('stn_advancedmoneywash:removecopblip')
AddEventHandler('stn_advancedmoneywash:removecopblip', function()
		RemoveBlip(copblip)
end)


RegisterNetEvent('stn_advancedmoneywash:setcopblip')
AddEventHandler('stn_advancedmoneywash:setcopblip', function(cx,cy,cz)
		RemoveBlip(copblip)
...
AddEventHandler('esx:onPlayerDeath', function(data)
    TriggerEvent("stn_advancedmoneywash:abortmission")
end)

RegisterNetEvent("stn_advancedmoneywash:abortmission")
AddEventHandler('stn_advancedmoneywash:abortmission', function()
	isTaken = 0
	startedjob = false
	TriggerServerEvent('stn_advancedmoneywash:registerActivity', 0)
	DeletePed(MoneyWashped2)
	DeletePed(MoneyWashped3)
	DeletePed(MoneyWashped4)
	RemoveBlip(deliveryblip)
	TriggerServerEvent('stn_advancedmoneywash:cooldown')
end)	