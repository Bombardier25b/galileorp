...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	PlayerData = xPlayer
	ESX.TriggerServerCallback('esx_tunercustom:getVehiclesPrices', function(vehicles)
		Vehicles = vehicles
	end)
end)
...
RegisterNetEvent('esx_tunercustom:installMod')
AddEventHandler('esx_tunercustom:installMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
	myCar = ESX.Game.GetVehicleProperties(vehicle)
	TriggerServerEvent('esx_tunercustom:refreshOwnedVehicle', myCar)
end)

RegisterNetEvent('esx_tunercustom:cancelInstallMod')
AddEventHandler('esx_tunercustom:cancelInstallMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
...
function OpenLSMenu(elems, menuName, menuTitle, parent)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), menuName,
	{
		...
		for k,v in pairs(Config.Menus) do
			...
			if k == data.current.modType or isRimMod then
				...
				if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
					...
					TriggerEvent('esx_tunercustom:installMod')
				else
					...
					if isRimMod then
						...
						TriggerServerEvent("esx_tunercustom:buyMod", price)
		...
		menu.close()
		TriggerEvent('esx_tunercustom:cancelInstallMod')
...
function OpenLSMenu2(elems, menuName, menuTitle, parent)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), menuName,
	{
		...
		for k,v in pairs(Config.Spray) do
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			local lastZone    = nil
			local respray = false
			if (PlayerData.job ~= nil and PlayerData.job.name == 'tuner') or Config.IsMechanicJobOnly == false then
			...
			if IsControlJustReleased(0, Keys['E']) and not lsMenuIsShowed and isInLSMarker and respray == false then
				if (PlayerData.job ~= nil and PlayerData.job.name == 'tuner') or Config.IsMechanicJobOnly == false then
					lsMenuIsShowed = true
			...
			if IsControlJustReleased(0, Keys['E']) and not lsMenuIsShowed and isInLSMarker and respray == true then
				print("das")
				if (PlayerData.job ~= nil and PlayerData.job.name == 'tuner') or Config.IsMechanicJobOnly == false then
					lsMenuIsShowed = true
